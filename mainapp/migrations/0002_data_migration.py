...
def forwards_func(apps, schema_editor):
    ...
    News.objects.create(
        ...
        body="Python предназначен для разработки и изучения новых приложений. Мы предлагаем непросто изучить новый язык программирования, но и научиться применять его на практике. Этот курс полезен не только тем, кто хочет научиться программировать, но и всем тем, кому не хватает времени на выполнение практических заданий, а также тем, у кого нет возможности посещать курсы. Курс состоит из 7 встреч — одно занятие в неделю длительностью 3 часа.\r\nВстречи проводятся по воскресеньям с 14:00.",
    )
    News.objects.create(
        ...
        body="Сегодня в Саратове не будет проходить открытый урок по PHP, который должен был состояться в среду на факультете компьютерных наук СГУ. Преподавательница, за которой закреплена аудитория, сказала что заболела и не придёт. На следующий урок, запланированный на 28 февраля, перенесли всё, кроме курса «Веб-программирование». Об этом сообщили в группе факультета в социальной сети «ВКонтакте». Курс «Веб-программист» состоит из двух частей. Первая — общий курс, идущий в течение 72 часов.",
    )
    News.objects.create(
        ...
        body="Каждый из 5 руководителей будет в одном из своих офисов с использованием этого приложения. Мы хотим, чтобы они могли видеть всё, что происходит на сайте, просматривать отчёты, которые им предоставляются. После того как сотрудники подключили ZOOM, нам важно, чтобы каждый секретарь, менеджер и прочие были подключены ко всем 5 компьютерам.",
    )
    News.objects.create(
